...
// IsCIFSMount 检查给定路径的挂载是否为 CIFS/SMB 挂载
func IsCIFSMount(mountPath string) (bool, error) {
	info, err := GetMountInfo(mountPath)
	if err != nil {
		return false, err
	}

	return info.FSType == "cifs", nil
}

// GetMountedCIFSShares 返回基础目录下所有已挂载的 CIFS 共享
func GetMountedCIFSShares(baseDir string) ([]string, error) {
	mounts, err := mountinfo.GetMounts(mountinfo.FSTypeFilter("cifs"))
	if err != nil {
		return nil, fmt.Errorf("failed to get CIFS mounts: %w", err)
	}

	var shares []string
	for _, m := range mounts {
		// Check if this mount is under our base directory
		if isSubPath(m.Mountpoint, baseDir) {
			shares = append(shares, m.Mountpoint)
		}
	}
